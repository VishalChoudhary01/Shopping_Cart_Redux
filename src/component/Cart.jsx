    ...
    return (
        <>
            {isOpen && (
                ...
                >
                    <section
                        className={`fixed top-0 right-0 w-1/4 h-full bg-white shadow-lg z-50 transform transition-transform duration-300 ${
                        ...
                        <section className="p-4">
                            ...
                            {cartItems.length === 0 ? (
                                ...
                                <ul className="list-disc mt-4 w-full">
                                    {cartItems.map((item) => (
                                        <li key={item.id} className="flex items-center justify-between py-2 border-b">
                                            <section className="w-[70%]">
                                                ...
                                                <section className="">
                                            <p className="text-[0.8em] font-medium text-gray-600"><span>₹</span>{item.price} X {item.quantity} = <span>₹</span> <span>{item.totalPrice}</span></p> 
                                                </section>
                                            </section>
                                    ...
                                    <section className="text-lg text-gray-700 font-semibold">
                                        Grand Total: <span>₹</span> <span>{totalPrice}</span>
                                    </section>
                                </ul>
                            )}
                        </section>
